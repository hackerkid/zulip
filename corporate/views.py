...
def update_plan(request: HttpRequest, user: UserProfile,
                status: Optional[int]=REQ("status", validator=check_int, default=None),
                licenses: Optional[int]=REQ("licenses", validator=check_int, default=None),
                licenses_at_next_renewal: Optional[int]=REQ("licenses_at_next_renewal", validator=check_int, default=None)
                ) -> HttpResponse:
    ...
    if new_plan is not None:
        return json_error(_("Unable to update the plan. The plan has been expired and replaced with a new plan."))
    ...
    if licenses is not None:
        if plan.automanage_licenses:
            return json_error(_("Unable to update licenses manually. Your plan is on automatic license management."))
        if last_ledger_entry.licenses == licenses:
            return json_error(_("Your plan is already on {licenses} licenses in the current billing period.").format(licenses=licenses))
        if last_ledger_entry.licenses > licenses:
            return json_error(_("You cannot decrease the licenses in the current billing period.").format(licenses=licenses))
        try:
    ...
    if licenses_at_next_renewal is not None:
        ...
        if last_ledger_entry.licenses_at_next_renewal == licenses_at_next_renewal:
            return json_error(_("Your plan is already scheduled to renew with {licenses_at_next_renewal} licenses.").format(licenses_at_next_renewal=licenses_at_next_renewal))
        try:
            validate_licenses(plan.charge_automatically, licenses_at_next_renewal, get_latest_seat_count(user.realm))
        except BillingError as e:
            return json_error(e.message, data={'error_description': e.description})
