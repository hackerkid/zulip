...
def update_plan(request: HttpRequest, user: UserProfile,
    ...
    if new_plan is not None:
        return json_error(_("Unable to update the plan. The plan has expired and replaced with a new plan."))
    ...
    if licenses is not None:
        ...
        if last_ledger_entry.licenses == licenses:
            return json_error(_("Your plan is already on {licenses} licenses.").format(licenses=licenses))
        try:
            validate_licenses(plan.charge_automatically, licenses, get_latest_seat_count(user.realm))
        except BillingError as e:
            return json_error(e.message, data={'error_description': e.description})
        update_license_ledger_for_manual_plan(user.realm, plan, licenses, timezone_now())
        return json_success()
